(function () {
  ...
  let dragOffset = { x: 0, y: 0 }; // Offset for dragging // Asynchronous function to process the prompt text
  ...
  function createPopup(text) {
    ...
    if (popupElement) return; // Create the popup element
    ...
    popupElement.innerHTML = `<p>${text}</p>`; // Create the close button
    ...
    closeButton.style.color = "#ffffff"; // Append the close button to the popup

    popupElement.appendChild(closeButton); // Append the popup to the body

    document.body.appendChild(popupElement); // Use setTimeout to trigger the fade-in effect

    setTimeout(() => {
      popupElement.style.opacity = "1";
    }, 10); // Add event listener to close the popup when the close button is clicked

    closeButton.addEventListener("click", closePopup); // Add event listeners for dragging
  ...
  function createUppercaseButton(input) {
    ...
    button.id = `uppercase-button-${uniqueId}`; // Assign unique ID to the button // Fetch the image from extension resources using chrome.runtime.getURL

    const imageUrl = chrome.runtime.getURL("roundlogo.png"); // Use an image as the button content

    button.innerHTML = `<img src="${imageUrl}" alt="UPPERCASE" style="width: 24px; height: 24px;" />`; // Style the button
    ...
    button.style.marginLeft = "8px"; // Margin for placement next to input // Position the button relative to the input

    const inputRect = input.getBoundingClientRect();
    button.style.top = `${inputRect.top}px`; // Position at the top of the input
    button.style.left = `${inputRect.right - 50}px`; // Position to the right of the input // Add the click event listener to handle popup display and text transformation
  ...
  function setupInputListener(input) {
    if (input && !input.dataset.listenerAdded) {
      ...
      if (button) {
        positionButtonRelativeToInput(button, input); // Reposition the button on window resize or scroll
      ...
      input.addEventListener("input", (event) => {
        ...
        console.log("Current prompt text:", promptText); // Clear previous timer and start a new one for debouncing
          ...
          chrome.storage.local.get("isUppercaseEnabled", async function (data) {
            ...
            if (data.isUppercaseEnabled) {
              try {
                ...
                console.log("Processed text:", processedText); // Update the input with the processed text
  ...
  findAndSetupInput(); // Set up a MutationObserver to detect dynamically added inputs
  ...
  observer.observe(document.body, { childList: true, subtree: true }); // Cleanup function to disconnect the MutationObserver on page unload
